...
use eventlog_core::{
    AppendGroup, CommandMeta, EventLogError, Expected, NewEvent, RecordedEvent, StreamAppend,
    StreamId, TenantId, request_hash,
};
...
use crate::wire::{IdentityClaim, Payload, StoredRecord, Subject, decode, invalid};
...
pub(crate) struct RecordedStore<S> {
    pub(crate) store: S,
    pub(crate) tenant: TenantId,
    pub(crate) history_type: String,
    identity_type: String,
    subject: String,
    actor: String,
    cache: BTreeMap<(String, String), (Option<eventlog_core::SnapshotGeneration>, Subject)>,
    pub(crate) documents_ready: bool,
...
impl<S: StoreIo> RecordedStore<S> {
    ...
    pub(crate) fn history_stream(&self, entity: &str, id: &str) -> Result<StreamId, StoreError> {
        StreamId::new(
            self.tenant.clone(),
    ...
    async fn subjects(&mut self) -> Result<Vec<(String, String)>, StoreError> {
    ...
    async fn identity_claim(
        &mut self,
        payload: &Payload,
    ) -> Result<Option<IdentityClaim>, StoreError> {
...
impl<S: StoreIo> AsyncRecordedStore for RecordedStore<S> {
...
impl<S: StoreIo> AsyncAtomicRecordedStore for RecordedStore<S> {
...
pub(crate) fn digest(value: &impl Serialize) -> Result<String, StoreError> {
...
pub(crate) fn backend(error: EventLogError) -> StoreError {
    match error {
...
impl<S> RecordedStore<S> {
    pub(crate) fn with_io<T>(&self, store: T) -> RecordedStore<T> {
        RecordedStore {
            store,
            tenant: self.tenant.clone(),
            ...
            cache_subjects: self.cache_subjects,
