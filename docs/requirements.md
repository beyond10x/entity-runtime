...
| id | requirement | pinned by |
|---|---|---|
| R-20 | ✎ Field kinds: `string`, `integer`, `number`, `boolean`, `enum`, `array`, `object`, `json`. Each is type-checked except `json`, which accepts anything. An integer outside `i64` is compared numerically, never wrapped. | `every_field_kind_is_checked`, `an_integer_beyond_i64_is_compared_numerically_not_wrapped` |
...
| R-26 | ✚ A constraint declared on a kind it does not apply to — `values` on a `string`, `items` on an `object`, `min_length` on an `integer` — is refused at registration rather than silently ignored. | `a_constraint_that_does_not_apply_to_its_kind_is_refused` |
...
| R-95 | ✚ Generated output is escaped for the format it is generated in: a state or operation name carrying a quote or a backslash produces valid DOT, never a graph with attributes nobody wrote. | `graph_dot_quotes_a_name_that_would_otherwise_close_the_string` |
