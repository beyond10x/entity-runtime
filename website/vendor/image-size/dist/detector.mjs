...
var HEIF = {
  ...
  calculate(input) {
    ...
    while (currentOffset < ipcoBox.offset + ipcoBox.size) {
      ...
      if (!ispeBox) break;
      if (ispeBox.size < 8) {
        throw new TypeError("Invalid HEIF, ispe box smaller than its header");
      }
  ...
  calculate(input) {
    ...
    while (imageOffset < fileLength && imageOffset < inputLength) {
      ...
      if (imageHeader[1] < SIZE_HEADER2) {
        throw new TypeError("Invalid ICNS, entry length below the entry header size");
      }
...
function extractPartialStreams(input) {
  ...
  while (offset < input.length) {
    ...
    if (!jxlpBox) break;
    if (jxlpBox.size < 12) {
      throw new TypeError("Invalid JXL, jxlp box smaller than its header");
    }
    partialStreams.push(
